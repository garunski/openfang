...
pub fn operation_cost(method: &str, path: &str) -> NonZeroU32 {
    match (method, path) {
        ...
        ("GET", p) if p.starts_with("/api/logs/files") => NonZeroU32::new(5).unwrap(),
        ("GET", p) if p.contains("/conduit-runs/") && p.ends_with("/trace") => {
...
mod tests {
    ...
    fn test_costs() {
        ...
        assert_eq!(operation_cost("GET", "/api/logs/files").get(), 5);
        assert_eq!(operation_cost("GET", "/api/logs/files/daemon").get(), 5);
